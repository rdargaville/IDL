pro peak_offpeak,dates,state,peak_or_not
...
ndays = 48L*(allend-allstart+1) ; 30 minute time resolution
...
ndays = ntime/48
peak_or_not = intarr(ntime)
d = 0
icount = 0L
for ix = 0L,ndays-1 do BEGIN
    for iy  = 0,47 do BEGIN
        peak_or_not(icount) = dow_index(d)
        icount = icount + 1
    endfor
    d = d + 1
    if d eq 7 then d = 0
...
if nii gt 0 then peak_or_not(ii) = 2
...
; easter dates
for i = 0,n_elements(easter)-1 do BEGIN
...
for i = 0,n_elements(qb)-1 do BEGIN
    kk = where(time gt qb(i) and time lt qb(i)+1)
    peak_or_not(kk) = 2
endfor

; labour day 
if state eq 'VIC' or state eq 'TAS' or  state eq 'VIC1' or state eq 'TAS1'  then BEGIN ; first Monday in June
labour = [julday(3,13,2000),julday(3,12,2001),julday(3,11,2002),$
    ...
    for i = 0,n_elements(labour)-1 do BEGIN
        kk = where(time gt labour(i) and time lt labour(i)+1)
        peak_or_not(kk) = 2
    endfor
endif
 
if state eq 'NSW' or state eq 'SA' or state eq 'NSW1' or state eq 'SA1' then BEGIN ; First Monday in October
...
if state eq 'QLD' or state eq 'QLD1' then BEGIN ; first Monday in May until 2013, then first in October
...
for i = 0,n_elements(mc)-1 do BEGIN
