pro color_bar_ucsb,levels,colors,pos=pos,subnorm=subnorm,title=title, $
              labels=labels,digits=digits,stride=stride,charsize=charsize,orientation=orientation
...
for i=0,ncolors-1 do begin
  xi=x1+dx*float(i)/ncolors
  polyfill,xi+[0,dx,dx,0]/ncolors+i/ncolors,y1+[0,0,dy,dy], $
     color=colors(i),/device
endfor
...
for i=0,nlevels-1 do plots,xv(i)*[1,1],yv,/device

if nlabels eq 0 then begin
  fmt='(f20.2)'
  if n_elements(digits) ne 0 then begin
    if digits eq 0 then fmt='(i20)' $
                   else fmt='(f20.'+string(format='(i1,a)',digits,')')
  endif
  ticlab=string(format=fmt,levels)
endif else begin
  ticlab=labels
endelse
...
xyouts,xv,yv,ticlab,align=.5,charsize=lcharsize,/device,orientation=orientation
...
if keyword_set(title) then begin
  ...
  xyouts,x1+.5*dx,yt,title,align=.5,/device,charsize=tcharsize;, charthick=2
endif
